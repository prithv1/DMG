...
class Aggregate_Trainer:
    def __init__(
        ...
        # Criterion to GPU
        if self.params.PROCESS.USE_GPU:
            self.model.loss_gpu(True)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (image, label, _) in enumerate(self.train_split_obj.curr_loader):
            label = label.long()
            if self.cuda_flag:
                image = image.cuda()
                label = label.cuda()
    ...
    def validate(self, mode="val"):
        ...
        for batch_idx, (image, label, _) in enumerate(rel_loader):
            label = label.long()
            if self.cuda_flag:
                image = image.cuda()
                label = label.cuda()
    ...
    def train(self, domain_list=None, ckpt_store_interval=20):
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            # Evaluate on the train and val splits
            # 1. Training Split
            ...
            # WANDB Logs
            # Log losses of all domains in a single plot (both train + val)
            wandb_loss_log = {}
            ...
            # Log accuracies of all domains in a single plot (both train + val)
            ...
            # Update to match the DomainNet Multi-Source baselines
            if self.scheduler is not None:
            ...
            if best_score is None or score > best_score:
                ...
                best_test_score_wandb_table = wandb.Table(
                    columns=["Domain", "Test Loss", "Test Accuracy"]
                )
                for key, val in running_ts_loss_dict.items():
                    best_test_score_wandb_table.add_data(
                        key,
                        str(running_ts_loss_dict[key]),
                        str(running_ts_acc_dict[key]),
                    )
                wandb.log({"Best_Test_Score_Table": best_test_score_wandb_table})
